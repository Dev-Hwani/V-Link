...
- 토큰 재발급(회전): `POST /auth/refresh`
- 로그아웃(현재 세션): `POST /auth/logout`
- 개별 세션 로그아웃: `POST /auth/logout-session`
- 내 세션 목록: `GET /auth/sessions`
- 전체 로그아웃(내 모든 세션 종료): `POST /auth/logout-all`
- 관리자 전용 사용자 등록: `POST /auth/register`
- 역할별 접근 제어(RBAC) 및 데이터 격리 적용
- 로그인/회원가입 요청 횟수 제한(429, IP 기반) 적용
- 로그인 실패 누적 잠금(기본 5회 실패 시 15분 잠금)
- 비밀번호 정책: 8자 이상 + 영문 대문자/소문자/숫자/특수문자 포함
...
- 인증 보안: `AUTH_LOGIN_RATE_LIMIT_*`, `AUTH_SIGNUP_RATE_LIMIT_*`, `AUTH_LOGIN_MAX_FAILED_ATTEMPTS`, `AUTH_LOGIN_LOCK_MINUTES`
...
- Refresh Token은 회전 방식으로 동작하며, 재사용 감지 시 해당 사용자의 활성 세션을 즉시 폐기합니다.
- 만료/폐기 토큰은 1시간 주기 cron으로 정리되며 보관 기간은 `REFRESH_TOKEN_REVOKED_RETENTION_DAYS`로 조정할 수 있습니다.
